...
const MyAddedCourses = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://online-learning-platform-server-orpin.vercel.app/courses/instructor/${user.email}`
      );
  ...
  const handleDelete = async (courseId) => {
    ...
    try {
      const response = await fetch(
        `https://online-learning-platform-server-orpin.vercel.app/delete-course/${courseId}`,
        { method: "DELETE" }
      );
    ...
    >
      <div className="container mx-auto px-4 pt-20 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <Link
            to="/add-course"
            className="btn bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white rounded-lg px-6 py-2 transition-all"
          >
            Add New Course
          </Link>
        </div>
        ...
        {courses.length === 0 ? (
          ...
          >
            ...
            <Link
              to="/add-course"
              className="btn bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white rounded-lg px-6 py-2"
            >
              Add Course
            </Link>
          </div>
          ...
          <div className="overflow-x-auto rounded-lg shadow-md">
            <table
              className={`table w-full ${isDark ? "bg-gray-800" : "bg-white"}`}
            >
              <thead
                className={`${
                  isDark
              ...
              <tbody>
                {courses.map((course) => (
                  ...
                  >
                    <td
                      className={`font-medium px-4 py-4 align-top ${
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className="flex gap-5">
                        ...
                        <Link
                          ...
                          className="flex items-center gap-1 text-yellow-500 hover:text-yellow-600 font-medium transition-colors"
                        >
                          <FaEdit /> Edit
                        </Link>
                        <button
        ...
        {deleteModal.isOpen && (
          ...
          >
            <div
              className={`${
                isDark ? "bg-gray-800" : "bg-white"
              } rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all`}
            ...
            >
              ...
              >
                ...
                be undone and all enrolled students will lose access.
              </p>
